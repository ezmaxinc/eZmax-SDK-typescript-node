...
export class ObjectEzsigndocumentApi {
     ...
     */
    public async ezsigndocumentEditObjectV1 (pkiEzsigndocumentID: number, ezsigndocumentEditObjectV1Request: EzsigndocumentEditObjectV1Request, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: EzsigndocumentEditObjectV1Response;  }> {
        const localVarPath = this.basePath + '/1/object/ezsigndocument/{pkiEzsigndocumentID}'
            .replace('{' + 'pkiEzsigndocumentID' + '}', encodeURIComponent(String(pkiEzsigndocumentID)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(ezsigndocumentEditObjectV1Request, "EzsigndocumentEditObjectV1Request")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: EzsigndocumentEditObjectV1Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "EzsigndocumentEditObjectV1Response");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @param pkiEzsigndocumentID The unique ID of the Ezsigndocument
     */
    public async ezsigndocumentGetObjectGetChildrenV1 (pkiEzsigndocumentID: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
